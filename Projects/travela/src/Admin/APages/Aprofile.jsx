...
function Aprofile() {
    ...
    // Fetch user data
    useEffect(() => {
        axios.get("http://localhost:3000/admin/1")
            .then((res) => {
                setadminp(res.data);
    ...
    const handleUpdate = async () => {
        try {
            const res = await axios.put(`http://localhost:3000/admin/${updatedData.id}`, updatedData);
        ...
        <>
            <div className="profile-container">
                <div className="profile-card">
                    <img className="profile-pic" src={adminp.img} alt="Profile" />
                    <h2>{adminp.name || "Loading..."}</h2>
                    <p>{adminp.email || "Loading..."}</p>
